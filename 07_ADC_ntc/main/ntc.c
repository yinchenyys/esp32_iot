...
void temp_ntc_init(void)
{
    ...
    //1.创建一个单通道的adc转换adc_oneshot_new_unit
    ESP_ERROR_CHECK(adc_oneshot_new_unit(&init_config1, &s_adc_handle));//启用ADC单次转换

    //-------------ADC1 Config---------------//
    adc_oneshot_chan_cfg_t config = {
        .bitwidth = ADC_BITWIDTH_12,       //分辨率 2^12-1=4095
        .atten = ADC_ATTEN_DB_12,          //衰减倍数，由于esp32只能采样1.1v,所以要衰减
    ...
    //2.配置单通道adc_oneshot_config_channel
    ESP_ERROR_CHECK(adc_oneshot_config_channel(s_adc_handle, TEMP_ADC_CHANNEL, &config));

    //-------------ADC1 Calibration Init---------------//
    //3.判断是否需要校准example_adc_calibration_init
    do_calibration1 = example_adc_calibration_init(ADC_UNIT_1, ADC_ATTEN_DB_12, &adc1_cali_handle);//自定义函数，判断是否需要校准
...
static void temp_adc_task(void* param)
{
    ...
    while(1)
    {
        ...
        if(adc_cnt >= 10)
        {
            ...
            {
                //4.电压转换为相应的电阻值（二分查找）
                res = (voltage*NTC_RES)/(ADC_V_MAX-voltage);// voltage /   ((ADC_V_MAX-voltage)/NTC_RES) 
                //根据电阻值查表找出对应的温度
...
{//由于不好计算所以使用二分查找，找到相似值，再把剩下差值到下一个只视为等比进行估算
    // uint16_t middle = (left + right) / 2;
    // if (right <= left || left == middle)//一般都会走left == middle，小于最小值才会right < left，一般不会有right = left
    // {
    //     if (res >= s_ntc_table[left].res)//不超过最大值都走这条
    //         return left;
    //     else
    //         return right;
    // }
    ...
    uint16_t middle_cnt = (right + left) /2;
    ...
    if(middle_cnt == left) return left;
    else if(s_ntc_table[middle_cnt].res < res  ) return find_ntc_index(res,left,middle_cnt);
    else if(s_ntc_table[middle_cnt].res > res  ) return find_ntc_index(res,middle_cnt,right);
    return 0;
...
static float get_ntc_temp(uint32_t res)
{
    ...
    int index = find_ntc_index(res,left,right);//先查表
    if (res == s_ntc_table[index].res)
        return s_ntc_table[index].temp;
    if (index == 0 || index == s_us_ntc_table_num - 1)//最大值或最小值
    {
        return s_ntc_table[index].temp;
    }
    else
