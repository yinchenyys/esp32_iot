...
esp_err_t st7789_driver_hw_init(st7789_cfg_t* cfg)
{
    ...
    if(cfg->rst > 0)
    {
        ...
        //相比于iot多了这部分，而且iot是用回调一个点一个点画出来的，这里是给lvgl传io_handle
    const esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = cfg->rst,
        .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,//￥选择渲染方式是RGB，还是GBR
        .bits_per_pixel = 16,//定义颜色深度
    };
    ESP_GOTO_ON_ERROR(esp_lcd_new_panel_st7789(lcd_io_handle, &panel_config, &panel_handle), err, TAG, "New panel failed");
    
    esp_lcd_panel_reset(panel_handle);  // 液晶屏复位
    // pca9557_set_GPIO(PCA9557_GPIO_lcd_cs,0);//有配置就不需要手动了
    esp_lcd_panel_init(panel_handle);  // 初始化配置寄存器
    esp_lcd_panel_invert_color(panel_handle, true); // 颜色反转

    #if show_y
        #if show_f 
         // esp_lcd_panel_swap_xy(panel_handle, true);  // ￥显改为横屏 //摄像头为上为正
        esp_lcd_panel_mirror(panel_handle, true, true); // 镜像       
        #else
        // esp_lcd_panel_swap_xy(panel_handle, true);  // ￥显改为横屏 //摄像头为上为正
