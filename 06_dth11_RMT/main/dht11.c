...
static bool IRAM_ATTR example_rmt_rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata, void *user_data)
{
    ...
    QueueHandle_t rx_receive_queue = (QueueHandle_t)user_data;//这句可以省去，因为有一个全局rx_receive_queue，不用再穿过来
    // 把数据放进队列
...
void DHT11_Init(uint8_t dht11_pin)
{
    ...
    rmt_rx_event_callbacks_t cbs = {
        .on_recv_done = example_rmt_rx_done_callback,//接受完成的回调函数
    };

	//1.注册rmt读取回调函数rmt_rx_register_event_callbacks
    ESP_ERROR_CHECK(rmt_rx_register_event_callbacks(rx_chan_handle, &cbs, rx_receive_queue));

	//2.使能RMT接收通道rmt_enable
...
{
	//1.唤醒DTH11
	//（1）.设置引脚为输出口，并先拉高1us准备
	gpio_set_direction(DHT11_PIN, GPIO_MODE_OUTPUT);
	gpio_set_level(DHT11_PIN, 1);
	ets_delay_us(1000);//等待稳定
	...
	// 3.等待消息队列数据约10ms，完成则调用处理函数返回
	static rmt_rx_done_event_data_t rx_data;	//实际接收到的数据0（接受完成RMT的回调函数数据类型）
