...
{
    if(s_flush_done_cb)
        s_flush_done_cb(user_ctx);//port写的函数，调用lv_disp_flush_ready(&disp_drv)，开启disp,参数是初始化disp后的结构体
    return false;
}
...
esp_err_t st7789_driver_hw_init(st7789_cfg_t* cfg)
{
    ...
    //3.创建基于spi的lcd操作句柄esp_lcd_new_panel_io_spi
    esp_lcd_panel_io_spi_config_t io_config = {//spi引脚不归在内？
        .dc_gpio_num = cfg->dc,         //DC引脚（指令还是数据，1命令，0数据）
        .cs_gpio_num = cfg->cs,         //CS引脚，片选引脚
        .pclk_hz = cfg->spi_fre,        //SPI时钟频率
        .lcd_cmd_bits = 8,              //命令长度
        .lcd_param_bits = 8,            //参数长度
        .spi_mode = 0,                  //使用SPI0模式，4种模式，第一个位1表示默认高电平，第二为1表示第一个跳变就读取数据
        .trans_queue_depth = 10,        //表示可以缓存的spi传输事务队列深度，即最大可以有10组数据等待写入
        .on_color_trans_done = notify_flush_ready,   //刷新完成回调函数（回调函数会调用port传过来的回调函数）
        .user_ctx = cfg->cb_param,                                    //回调函数参数(由port传过来配置好的lv_disp结构体)
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_SPI_HOST, &io_config, &lcd_io_handle));//id做handle
    
    //5.硬件复位拉低20ms,再拉高20ms
    if(cfg->rst > 0)
    {
    ...
    /*向LCD写入初始化命令*/
    //6.使用lcd函数发送指令，进行初始化esp_lcd_panel_io_tx_param
    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_SWRESET,NULL,0);    //（1）.软件复位（这些指令都是哪来的，lcd都共用这些指令？）
    vTaskDelay(pdMS_TO_TICKS(150));
    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_SLPOUT,NULL,0);     //（2）.退出休眠模式
    vTaskDelay(pdMS_TO_TICKS(200));
    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_COLMOD,(uint8_t[]) {0x55,}, 1);  //（3）.选择RGB数据格式，0x55:RGB565,0x66:RGB666
    esp_lcd_panel_io_tx_param(lcd_io_handle, 0xb0, (uint8_t[]) {0x00, 0xF0},2);

    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_INVON,NULL,0);     //（4）.颜色翻转
    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_NORON,NULL,0);     //（5）.普通显示模式
    uint8_t spin_type = 0;
    switch(cfg->spin)
    {
        case 0:
    ...
    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_MADCTL,(uint8_t[]) {spin_type,}, 1);   //（6）.屏旋转方向
    vTaskDelay(pdMS_TO_TICKS(150));
    esp_lcd_panel_io_tx_param(lcd_io_handle,LCD_CMD_DISPON,NULL,0);    //（7）.打开显示
...
void st7789_flush(int x1,int x2,int y1,int y2,void *data)
{
    ...
    esp_lcd_panel_io_tx_param(lcd_io_handle, LCD_CMD_CASET, (uint8_t[]) {
        (x1 >> 8) & 0xFF,
        x1 & 0xFF,//前两个是起始行的高8位和低8位
        ((x2 - 1) >> 8) & 0xFF,
        (x2 - 1) & 0xFF,//后两个是终止行的高8位和低8位
    }, 4);
