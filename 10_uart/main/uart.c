...
{
    uart_event_t event;//一个uart接收到后保存的数据类型
...
void app_main(void)
{
    ...
    uart_driver_install(USER_UART_NUM,1024,1024,20,&uart_queue,0);     //1.安装串口驱动uart_driver_install(最后一个起什么作用)
    uart_param_config(USER_UART_NUM,&uart_cfg);                        //2.配置uartuart_param_config
    uart_set_pin(USER_UART_NUM,GPIO_NUM_32,GPIO_NUM_33,UART_PIN_NO_CHANGE,UART_PIN_NO_CHANGE); //3.设置引脚,tx为32,rx为33,uart_set_pin
    
    xTaskCreatePinnedToCore(uart_event_task,"uart",4096,NULL,3,NULL,1);
    #if 0
    while(1)
    {//4.接受数据uart_read_bytes
        int rec = uart_read_bytes(USER_UART_NUM,uart_buffer,sizeof(uart_buffer),pdMS_TO_TICKS(100));//返回rec表示收到多少个字节，1024是上限（100ms收到多少返回多少）
        if(rec)
        {//5.发送数据uart_write_bytes
